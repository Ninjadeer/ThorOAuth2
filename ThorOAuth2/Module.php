...
ServiceProviderInterface
{
	...
	public function getServiceConfiguration()
	{
		return array(
		'factories' => array(
			'authorizationServer' => function ($sm) {
			$plugin = new Controller\Plugin\AuthorizationServer();
			$plugin->setServerService($sm->get('thorauth2_server_service'));
			return $plugin;
			},
			'thorauth2_client_mapper' => function ($sm) {
			$adapter = $sm->get('zfcuser_zend_db_adapter');
			$tg = new \Zend\Db\TableGateway\TableGateway('oauth2_client', $adapter);
			return new Model\ClientMapper($tg);
			},
			'thorauth2_auth_code_mapper' => function ($sm) {
			$adapter = $sm->get('zfcuser_zend_db_adapter');
			$tg = new \Zend\Db\TableGateway\TableGateway('oauth2_auth_codes', $adapter);
			return new Model\AuthCodeMapper($tg);
			},
			'thorauth2_server_service' => function ($sm) {
			$adapter = $sm->get('zfcuser_zend_db_adapter');
			$clientService = new ServerService();
			$clientService->setClientMapper($sm->get('thorauth2_client_mapper'));
			$clientService->setAuthCodeMapper($sm->get('thorauth2_auth_code_mapper'));
